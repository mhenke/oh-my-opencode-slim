...
| `image_routing` | `"auto"` \| `"direct"` | `"direct"` | Optional. `"direct"` (default, same as before): images pass to orchestrator. `"auto"`: save to disk, delegate to @observer (requires observer enabled). When omitted, defaults to `"direct"`. |
