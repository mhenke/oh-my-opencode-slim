...
| `image_routing` | `"auto"` \| `"direct"` | `"direct"` | Optional. How image attachments are handled. When omitted, defaults to `"direct"` — same behavior as before PR #723. `"direct"`: images pass to the orchestrator untouched. `"auto"`: save images to `.opencode/images/` and nudge the orchestrator to delegate to `@observer` (requires `observer` to be enabled, i.e. not in `disabled_agents`). |
